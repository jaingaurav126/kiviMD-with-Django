...
class AIChatbot(MDApp):
    ...
    def response(self, *args):
        ...
        name_response = ["I am AIChatbot. You can ask anything who to....",
                         "I am AIChatbot, an AI chatbot model", "You can call me AIChatbot"]
        ...
        elif 'local'.lower() in query.lower():
            response = self.get_local_weather()
            # print(response)
        else:
    ...
    def get_local_weather(self):
        ...
        try:
            response = requests.get('http://127.0.0.1:8000/information/')
            
            if response.status_code == 200:
                return response.json().get('weather_info', 'Weather information not available')
            else:
                return 'Failed to get weather information from the backend'
